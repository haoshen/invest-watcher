...
public class HoldService {
    ...
    public List<Hold> getHoldByConditionWithoutRecords(Integer userId, String investId, Integer direction, Integer status,
                                         Boolean usePaging, Integer limit, Integer offset) {
        // 不采用分页
        if(!usePaging) {
            return holdMapper.getHoldByConditionWithoutRecords(userId, investId, direction, status, null, null);
        }
        if (limit == null || limit < 0) {
            limit = 0;
        }
        ...
        return holdMapper.getHoldByConditionWithoutRecords(userId, investId, direction, status, offset, limit);
    }
